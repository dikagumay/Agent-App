angular.module('PruForce.factories')
	.factory('CandidateFactory', function ($q, $interval, $rootScope) {
		...
		var DataProvider = function ($q) {
			...
			function GetDataFromWebAPI(request, flagonline) {
				...
				if (flagonline) {
					...
					//WL.Client.invokeProcedure(request, APIEventOnline);
				} else {
					resourceRequest.sendFormParameters(request.parameters).then(
						onApiSuccess,
						onApiFailure
			...
			function GetDataFromJsonStore(request, flagonline) {
				...
				try {
					...
					var query = { agentId: $rootScope.agent.code, npa: $rootScope.candidate.npa };
					var options = {
						exact: false
			...
			function getCollection(res) {
				...
				switch (procedureName) {
					...
					case "getDataApplicationPack":
						result = res.responseJSON.result;
						break;
					default:
			...
			function onApiSuccess(res) {
				...
				result.candidateNpa = $rootScope.candidate.npa;
				...
				data.npa = result.candidateNpa;
				AppsLog.log("onApiSuccessOnline" + JSON.stringify(getCollection(res)));
